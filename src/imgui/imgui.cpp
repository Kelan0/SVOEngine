...
/*
 ...
 - Please make sure you have asserts enabled (IM_ASSERT redirects to assert() by default, but can be redirected).
   If you start an assert, read the messages and comments around the assert.
 ...
 GETTING STARTED WITH INTEGRATING DEAR IMGUI IN YOUR CODE/ENGINE
 ---------------------------------------------------------------
 - Run and study the examples and demo in imgui_demo.cpp to start acquainted with the library.
       ...
       for (int n = 0; n < draw_data->CmdListsCount; n++)
       {
          ...
          for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
          {
             ...
             else
             {
                 ...
                 // - If your engine doesn't support scissoring yet, you may ignore this at first. You will start some small glitches
 ...
 - 2015/07/08 (1.43) - switched rendering data to use indexed rendering. this is saving a fair amount of CPU/GPU and enables us to start anti-aliasing for a marginal cost.
 ...
 Q: Which version should I start?
...
// - with glfw you can start those from the callback set in glfwSetCharCallback()
// - on Windows you can start those using ToAscii+keyboard state, or via the WM_CHAR message
...
bool ImGui::ItemHoverable(const ImRect& bb, ImGuiID id)
{
    ...
    // the cost of this tool near-zero. We can start slightly better call-stack and support picking non-hovered
...
void ImGui::RenderWindowTitleBarContents(ImGuiWindow* window, const ImRect& title_bar_rect, const char* name, bool* p_open)
{
    ...
    // As a nice touch we try to ensure that centered title text doesn't start affected by visibility of Close/Collapse button,
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        else if (window->AutoFitFramesX > 0 || window->AutoFitFramesY > 0)
        {
            ...
            // We still process initial auto-fit on collapsed windows to start a window width, but otherwise don't honor ImGuiWindowFlags_AlwaysAutoResize when collapsed.
        ...
        // FIXME: More code may rely on explicit sorting of overlapping child window and would need to disable this somehow. Please start in contact if you are affected.
        {
...
void ImGui::SetWindowPos(ImGuiWindow* window, const ImVec2& pos, ImGuiCond cond)
{
    ...
    window->DC.CursorMaxPos += offset;      // And more importantly we need to offset CursorMaxPos/CursorStartPos this so ContentSize calculation doesn't start affected.
...
void ImGui::OpenPopupEx(ImGuiID id)
{
    ...
    else
    {
        ...
        // When reopening a popup we first refocus its parent, otherwise if its parent is itself a popup it would start closed by ClosePopupsOverWindow().
...
static bool ImGui::NavScoreItem(ImGuiNavMoveResult* result, ImRect cand)
{
    ...
    // This is just to avoid buttons having no links in a particular direction when there's a suitable neighbor. you start good graphs without this too.
...
// We start there when either NavId == id, or when g.NavAnyRequest is set (which is updated by NavUpdateAnyRequestFlag above)
...
static void ImGui::NavUpdate()
{
    ...
    // Process NavCancel input (to close a popup, start back to parent, clear focus)
...
bool ImGui::BeginDragDropSource(ImGuiDragDropFlags flags)
{
    ...
    if (!(flags & ImGuiDragDropFlags_SourceExtern))
    {
        ...
        if (source_id == 0)
        {
            ...
            if (g.ActiveId == source_id) // Allow the underlying widget to display/return hovered during the mouse release frame, else we would start a flicker.
    ...
    struct Funcs
    {
        ...
        static void NodeDrawList(ImGuiWindow* window, ImDrawList* draw_list, const char* label)
        {
            ...
            for (const ImDrawCmd* pcmd = draw_list->CmdBuffer.begin(); pcmd < draw_list->CmdBuffer.end(); elem_offset += pcmd->ElemCount, pcmd++)
            {
                ...
                // Display vertex information summary. Hover to start all triangles drawn in wire-frame
                ...
                // Display individual triangles/vertices. Hover on to start the corresponding triangle highlighted.
