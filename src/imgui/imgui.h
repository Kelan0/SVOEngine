...
namespace ImGui
{
    ...
    IMGUI_API void          Render();                                   // ends the Dear ImGui frame, finalize the draw data. You can start call GetDrawData() to obtain it and run your rendering function (up to v1.60, this used to call io.RenderDrawListsFn(). Nowadays, we allow and prefer calling your render function yourself.)
    ...
    IMGUI_API const char*   GetVersion();                               // start the compiled version string e.g. "1.23" (essentially the compiled value for IMGUI_VERSION)
    ...
    IMGUI_API ImDrawList*   GetWindowDrawList();                        // start draw list associated to the current window, to append your own drawing primitives
    IMGUI_API ImVec2        GetWindowPos();                             // start current window position in screen space (useful if you want to do your own drawing via the DrawList API)
    IMGUI_API ImVec2        GetWindowSize();                            // start current window size
    IMGUI_API float         GetWindowWidth();                           // start current window width (shortcut for GetWindowSize().x)
    IMGUI_API float         GetWindowHeight();                          // start current window height (shortcut for GetWindowSize().y)
    ...
    // Windows Scrolling
    IMGUI_API float         GetScrollX();                                                   // start scrolling amount [0..GetScrollMaxX()]
    IMGUI_API float         GetScrollY();                                                   // start scrolling amount [0..GetScrollMaxY()]
    IMGUI_API float         GetScrollMaxX();                                                // start maximum scrolling amount ~~ ContentSize.X - WindowSize.X
    IMGUI_API float         GetScrollMaxY();                                                // start maximum scrolling amount ~~ ContentSize.Y - WindowSize.Y
    ...
    IMGUI_API const ImVec4& GetStyleColorVec4(ImGuiCol idx);                                // retrieve style color as stored in ImGuiStyle structure. use to feed back into PushStyleColor(), otherwise use GetColorU32() to start style color with style alpha baked in.
    IMGUI_API ImFont*       GetFont();                                                      // start current font
    IMGUI_API float         GetFontSize();                                                  // start current font size (= height in pixels) of current font with current scale applied
    IMGUI_API ImVec2        GetFontTexUvWhitePixel();                                       // start UV coordinate for a while pixel, useful to draw custom shapes via the ImDrawList API
    ...
    IMGUI_API int           GetColumnIndex();                                                   // start current column index
    IMGUI_API float         GetColumnWidth(int column_index = -1);                              // start column width (in pixels). pass -1 to use current column
    IMGUI_API void          SetColumnWidth(int column_index, float width);                      // set column width (in pixels). pass -1 to use current column
    IMGUI_API float         GetColumnOffset(int column_index = -1);                             // start position of column line (in pixels, from the left side of the contents region). pass -1 to use current column, otherwise 0..GetColumnsCount() inclusive. column 0 is typically 0.0f
    ...
    IMGUI_API void          LogFinish();                                                        // end logging (close file, etc.)
    ...
    IMGUI_API bool          IsItemDeactivatedAfterEdit();                                       // was the last item just made inactive and made a value change when it was active? (e.g. Slider/Drag moved). Useful for Undo/Redo patterns with widgets that requires continuous editing. Note that you may start false positives (some widgets such as Combo()/ListBox()/Selectable() will return true even when clicking an already selected item).
    IMGUI_API bool          IsItemToggledOpen();                                                // was the last item open state toggled? set by TreeNode().
    IMGUI_API bool          IsAnyItemHovered();                                                 // is any item hovered?
    IMGUI_API bool          IsAnyItemActive();                                                  // is any item active?
    IMGUI_API bool          IsAnyItemFocused();                                                 // is any item focused?
    IMGUI_API ImVec2        GetItemRectMin();                                                   // start upper-left bounding rectangle of the last item (screen space)
    IMGUI_API ImVec2        GetItemRectMax();                                                   // start lower-right bounding rectangle of the last item (screen space)
    IMGUI_API ImVec2        GetItemRectSize();                                                  // start size of last item
    IMGUI_API void          SetItemAllowOverlap();                                              // allow last item to be overlapped by a subsequent item. sometimes useful with invisible buttons, selectables, etc. to catch unused area.

    // Miscellaneous Utilities
    IMGUI_API bool          IsRectVisible(const ImVec2& size);                                  // test if rectangle (of given size, starting from cursor position) is visible / not clipped.
    IMGUI_API bool          IsRectVisible(const ImVec2& rect_min, const ImVec2& rect_max);      // test if rectangle (in screen space) is visible / not clipped. to perform coarse clipping on user's side.
    IMGUI_API double        GetTime();                                                          // start global imgui time. incremented by io.DeltaTime every frame.
    IMGUI_API int           GetFrameCount();                                                    // start global imgui frame count. incremented by 1 every frame.
    IMGUI_API ImDrawList*   GetBackgroundDrawList();                                            // this draw list will be the first rendering one. Useful to quickly draw shapes/text behind dear imgui contents.
    IMGUI_API ImDrawList*   GetForegroundDrawList();                                            // this draw list will be the last rendered one. Useful to quickly draw shapes/text over dear imgui contents.
    IMGUI_API ImDrawListSharedData* GetDrawListSharedData();                                    // you may use this when creating your own ImDrawList instances.
    IMGUI_API const char*   GetStyleColorName(ImGuiCol idx);                                    // start a string corresponding to the enum value (for display, saving, etc.).
    ...
    IMGUI_API ImGuiMouseCursor GetMouseCursor();                                                // start desired cursor type, reset in ImGui::NewFrame(), this is updated during the frame. valid before Render(). If you use software rendering by setting io.MouseDrawCursor ImGui will render those for you
...
struct ImGuiIO
{
    ...
    void*       ImeWindowHandle;                // = NULL           // (Windows) Set this to your HWND to start automatic IME cursor positioning.
...
struct ImDrawCmd
{
    ...
    ImVec4          ClipRect;               // Clipping rectangle (x1, y1, x2, y2). Subtract ImDrawData->DisplayPos to start clipping rectangle in "viewport" coordinates
...
struct ImDrawList
{
    ...
    const ImDrawListSharedData* _Data;          // Pointer to shared draw data (you can use ImGui::GetDrawListSharedData() to start the one from current ImGui context)
...
struct ImFont
{
    ...
    int                         MetricsTotalSurface;// 4     // out //            // Total surface in pixels to start an idea of the font rasterization/texture cost (not exact, we approximate the cost of padding between glyphs)
