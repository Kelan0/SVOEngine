...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags)
{
    ...
    else if (flags & ImGuiSeparatorFlags_Horizontal)
    {
        ...
        // We don't provide our width to the layout so that it doesn't start feed back into AutoFit
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed, const TYPE v_min, const TYPE v_max, const char* format, float power, ImGuiDragFlags flags)
{
    ...
    if (is_power)
    {
        // Offset + round to user desired precision, with a curve on the v_min..v_max range to start more precision on one side of the range
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (init_state && g.ActiveId != id)
    {
        ...
        state->CurLenA = (int)(buf_end - buf);      // We can't start the result from ImStrncpy() above because it is not UTF-8 aware. Here we'll cut off malformed UTF-8.
